...
function getPedidosHTML() {
  return '<div class="card"><div class="card__header"><h3>📋 Pedidos</h3><span id="pedidos-count" style="font-size:var(--font-size-sm);color:var(--color-ink-secondary);"></span></div><div class="card__body"><div class="filters-bar"><input type="text" class="form-input" id="filter-busca" placeholder="Buscar nome ou telefone…" oninput="debounceFilter()"><select class="form-input" id="filter-status" onchange="applyFilters()"><option value="">Todos os status</option><option value="pendente">Pendente</option><option value="aprovado">Aprovado</option><option value="processando">Processando</option><option value="concluido">Concluído</option><option value="cancelado">Cancelado</option><option value="erro">Erro</option></select><select class="form-input" id="filter-evento" onchange="applyFilters()"><option value="">Todos os eventos</option></select><input type="date" class="form-input" id="filter-data-inicio" onchange="applyFilters()" title="Data início"><input type="date" class="form-input" id="filter-data-fim" onchange="applyFilters()" title="Data fim"></div><div class="table-wrapper"><table><thead><tr><th>Cliente</th><th>Telefone</th><th>Evento</th><th>Status</th><th>Fotos</th><th>Data</th><th></th></tr></thead><tbody id="pedidos-tbody"><tr class="skeleton-row"><td><div class="skeleton skeleton--text" style="width:60%"></div></td><td><div class="skeleton skeleton--text-sm" style="width:45%"></div></td><td><div class="skeleton skeleton--text" style="width:50%"></div></td><td><div class="skeleton skeleton--text-sm" style="width:35%"></div></td><td><div class="skeleton skeleton--text-sm" style="width:20%"></div></td><td><div class="skeleton skeleton--text-sm" style="width:30%"></div></td><td><div class="skeleton skeleton--text-sm" style="width:25%"></div></td></tr></tbody></table></div><div class="pagination" id="pagination"><span id="pagination-info">Carregando…</span><div class="pagination__buttons" id="pagination-buttons"></div></div></div></div>';
}
...
function getPedidoDetalheHTML() {
  return '<div id="pedido-detail-loading" class="loading-overlay"><div class="spinner"></div><span>Carregando pedido…</span></div><div id="pedido-detail-content" class="hidden"><nav style="display:flex;align-items:center;gap:var(--space-3);margin-bottom:var(--space-6);flex-wrap:wrap;"><a href="#/pedidos" class="btn btn-sm btn-secondary">← Voltar</a><span id="detail-status" class="badge badge--info">Carregando…</span><span id="detail-id" style="font-size:var(--font-size-xs);color:var(--color-ink-muted);font-family:var(--font-mono);"></span></nav><div id="sse-status-bar" class="hidden alert alert--info" style="margin-bottom:var(--space-4);font-size:var(--font-size-xs);">🔄 Status atualizado em tempo real</div><div class="detail-grid"><div><div class="detail-section"><div class="detail-section__title">📸 Fotos Enviadas</div><div id="fotos-section"><div id="fotos-gallery" class="photo-gallery"></div><div id="fotos-empty" class="hidden"><div class="empty-state"><div class="empty-state__icon">🖼️</div><div class="empty-state__text">Nenhuma foto enviada</div></div></div></div></div><div class="detail-section"><div class="detail-section__title">🎯 Curadoria</div><div class="curation-tabs"><button class="curation-tab curation-tab--active" data-curation="all" onclick="filterCuration(\'all\')">Todas</button><button class="curation-tab" data-curation="selected" onclick="filterCuration(\'selected\')">Selecionadas</button><button class="curation-tab" data-curation="rejected" onclick="filterCuration(\'rejected\')">Rejeitadas</button></div><div id="curation-gallery" class="photo-gallery"></div></div></div><div><div class="detail-section"><div class="detail-section__title">👤 Cliente</div><div id="cliente-info"></div></div><div class="detail-section"><div class="detail-section__title">🎉 Evento</div><div id="evento-info"></div></div><div class="detail-section"><div class="detail-section__title">🎬 Vídeo</div><div id="video-section"><div id="video-empty" class="empty-state"><div class="empty-state__icon">🎥</div><div class="empty-state__text">Vídeo ainda não gerado</div><p>Após a curadoria e processamento, o vídeo aparecerá aqui.</p></div><div id="video-player-wrapper" class="hidden"><div class="video-player"><video id="video-player" controls preload="metadata"></video></div></div></div></div><div class="action-bar"><button class="btn btn-primary" onclick="reprocessarPedido()" id="btn-reprocessar">🔄 Reprocessar</button><button class="btn btn-danger" onclick="cancelarPedido()" id="btn-cancelar">🚫 Cancelar</button></div></div></div></div>';
}
