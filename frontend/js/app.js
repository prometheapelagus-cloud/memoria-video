...
function initLoginForm() {
  ...
  if (!form || !btn || !alertEl) { console.warn('[Login] Elementos do formulário não encontrados'); return; }
  form.addEventListener('submit', async (e) => {
    ...
    if (!email || !email.includes('@')) { if (emailErr) emailErr.classList.remove('hidden'); return; }
    if (!password || password.length < 4) { if (passErr) passErr.classList.remove('hidden'); return; }
    btn.disabled = true;
    ...
    try {
      ...
      if (alertEl) alertEl.innerHTML = '<div class="alert alert--error">'+escapeHtml(err.message||'Erro ao fazer login')+'</div>';
      btn.disabled = false;
